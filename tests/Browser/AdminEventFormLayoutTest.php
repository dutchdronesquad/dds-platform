...
test('the main admin navigation uses the orange brand accent', function () {
    ...
    $page = visit('/dashboard/events/create')
        ...
        ->assertScript(
            "(() => { const buttons = Array.from(document.querySelectorAll('[data-sidebar=\"sidebar\"] nav [data-sidebar=\"menu-button\"]')); const active = buttons.find((button) => button.getAttribute('aria-current') === 'page'); const inactive = buttons.filter((button) => button !== active); if (buttons.length !== 4 || active?.textContent?.trim() !== 'Events' || active.dataset.active !== 'true' || inactive.length !== 3) return false; const transparent = new Set(['transparent', 'rgba(0, 0, 0, 0)']); const activeStyle = getComputedStyle(active); const activeIcon = active.querySelector('svg'); return activeIcon !== null && !transparent.has(activeStyle.backgroundColor) && !transparent.has(activeStyle.borderLeftColor) && getComputedStyle(activeIcon).color === activeStyle.color && inactive.every((button) => { const style = getComputedStyle(button); const icon = button.querySelector('svg'); return button.dataset.active === 'false' && icon !== null && transparent.has(style.backgroundColor) && getComputedStyle(icon).color !== style.color; }); })()",
        );

    $page->script("document.documentElement.classList.add('dark')");

    $page->assertScript(
        "(() => { const active = document.querySelector('[data-sidebar=\"sidebar\"] nav [aria-current=\"page\"]'); if (active === null || active.dataset.active !== 'true' || !document.documentElement.classList.contains('dark')) return false; const transparent = new Set(['transparent', 'rgba(0, 0, 0, 0)']); const style = getComputedStyle(active); const icon = active.querySelector('svg'); return icon !== null && !transparent.has(style.backgroundColor) && !transparent.has(style.borderLeftColor) && getComputedStyle(icon).color === style.color; })()",
    );
